    ...
    private VerticalLayout addMenuDialog(Dialog dialog) {
        ...
        saveButton.addClickListener(e->{
            ...
            Boolean out = WebClient.create()
                    ...
                    .block();
            Notification addNoti = new Notification("เพิ่มเมนู "+titleField.getValue()+" แล้ว",2000);
            addNoti.addThemeVariants(NotificationVariant.LUMO_SUCCESS);
            addNoti.setPosition(Notification.Position.BOTTOM_END);
            getMenu();
            grid.setItems(menu.model);
            dialog.close();
        })
